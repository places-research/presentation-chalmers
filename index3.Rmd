...
if (!is.na(cvlong$type)[1] | length(unique(cvlong$type)) > 1) {
  ...
  #p <- p + geom_text(aes(x = label_pos))
   ...
   #  geom_label_repel(aes(x = label_pos), 
   #                         position = "dodge",
   # size = 4,
   # alpha = 0.9,
   # segment.size = .25,
...
df <- data.frame(event = as.character(event), 
...
cvlong <- data.frame(pos = rep(as.numeric(rownames(df)),2), 
                     ...
                     where = rep(as.character(df$Place),2), 
                     value = c(df$Start, df$End), 
                     label_pos = df$Start+ floor((df$End - df$Start)*2),2))
...
p <- ggplot(cvlong, aes(x = value, y = reorder(name, -pos), label = where, group = pos)) +
         ...
         geom_text_repel(data = cvlong %>% filter(type == "Heart"), mapping = aes(x = label_pos, label = where), min.segment.length = 0, ylim = c(0, NA), xlim = c(0, NA), hjust = -2) +
   geom_label(aes(x = label_pos),
    colour = "black",
    size = 2, alpha = 0.7) +      
   labs(title = "", subtitle = "", x = NULL, y = NULL, colour = NULL) +
   ...
   #+
   #      geom_text(aes(x = label_pos), hjust = 0, nudge_x = -1)
...
ggsave("cv_timeline.png", width = 12, height = 6)
...
plot_timeline2 <- function(event, start, end = start + 1, label = NA, group = NA,
  ...
  cvlong <- data.frame(pos = rep(as.numeric(rownames(df)),2), 
                       name = rep(as.character(df$Role), 2), 
                       type = rep(factor(df$Type, levels = c("Life", "Education", "Work\n Teaching and\n research", "Work\n industry", "Heart")), 2),                        where = rep(as.character(df$Place),2), 
                       value = c(df$Start, df$End), 
                       label_pos = rep(df$Start + floor((df$End - df$Start) / 2), 2))
